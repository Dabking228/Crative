...
def _make_analysis(
    ...
    for d in DIMS:
        scores = [
            g["dimensions"][d]["score"],
            c["dimensions"][d]["score"],
            k["dimensions"][d]["score"],
        ]
    ...
    return {
        ...
        "model_recommendations": {
            "gemini": {
                "recommendation": gemini_rec,
                "reasoning": _rec_reasoning(gemini_rec, "Gemini"),
                "error": False,
            },
            "claude": {
                "recommendation": claude_rec,
                "reasoning": _rec_reasoning(claude_rec, "Claude"),
                "error": False,
            },
            "grok": {
                "recommendation": grok_rec,
                "reasoning": _rec_reasoning(grok_rec, "Grok"),
                "error": False,
            },
        },
...
def _red_flags(dim: str, score: int) -> list:
    ...
    if score >= 5:
        return [
            f"Early-stage validation — more evidence needed for {dim.replace('_', ' ')}"
        ]
    return [
    ...
    return GraphState(
        ...
        ocr_structured_json={
            ...
            "directors": (
                [
                    {
                        "name": "Ahmad Razif bin Abdullah",
                        "ic_no": "850312-14-5678",
                    ...
                    {
                        "name": "Lim Wei Ming",
                        "ic_no": "880901-10-1234",
                        "nationality": "Malaysian",
                        "shareholding_pct": ownership - 60,
                        "is_resident": True,
                    },
                ]
                if ownership >= 60
                    ...
                    {
                        "name": "Dr Nurul Hana binti Ismail",
                        "ic_no": "791204-03-5432",
                        "nationality": "Malaysian",
                        "shareholding_pct": 45,
                        "is_resident": True,
                    },
                    {
                        "name": "Singapore Health Ventures Pte Ltd",
                        "nationality": "Foreign entity",
                        "shareholding_pct": 25,
                        "is_resident": False,
                    },
                ]
...
def build_demo_applications() -> Dict[str, GraphState]:
    ...
    apps["APPDEMO001"] = {
        **finpay,
        ...
        "gemini_analysis": finpay_analysis["dimensions"][
            "strengths"
    ...
    apps["APPDEMO001"]["gemini_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: finpay_analysis["dimensions"][d]["gemini"]
            for d in finpay_analysis["dimensions"]
        },
    ...
    apps["APPDEMO001"]["claude_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: finpay_analysis["dimensions"][d]["claude"]
            for d in finpay_analysis["dimensions"]
        },
        "overall_recommendation": "APPROVE",
    ...
    apps["APPDEMO001"]["grok_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: finpay_analysis["dimensions"][d]["grok"]
            for d in finpay_analysis["dimensions"]
        },
        "overall_recommendation": "APPROVE",
    ...
    medtech_analysis = _make_analysis(
        ...
        gemini_rec="NEEDS_MORE_INFO",
        claude_rec="NEEDS_MORE_INFO",
        grok_rec="APPROVE",
    )
    apps["APPDEMO002"] = {
        **medtech,
    ...
    apps["APPDEMO002"]["gemini_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: medtech_analysis["dimensions"][d]["gemini"]
            for d in medtech_analysis["dimensions"]
        },
    ...
    apps["APPDEMO002"]["claude_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: medtech_analysis["dimensions"][d]["claude"]
            for d in medtech_analysis["dimensions"]
        },
        "overall_recommendation": "NEEDS_MORE_INFO",
    ...
    apps["APPDEMO002"]["grok_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: medtech_analysis["dimensions"][d]["grok"]
            for d in medtech_analysis["dimensions"]
        },
        "overall_recommendation": "APPROVE",
        "recommendation_reasoning": _rec_reasoning("APPROVE", "Grok"),
    ...
    apps["APPDEMO003"]["gemini_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: agridrone_analysis["dimensions"][d]["gemini"]
            for d in agridrone_analysis["dimensions"]
        },
        "overall_recommendation": "APPROVE",
        "recommendation_reasoning": _rec_reasoning("APPROVE", "Gemini"),
    }
    apps["APPDEMO003"]["claude_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: agridrone_analysis["dimensions"][d]["claude"]
            for d in agridrone_analysis["dimensions"]
        },
        "overall_recommendation": "APPROVE",
        "recommendation_reasoning": _rec_reasoning("APPROVE", "Claude"),
    ...
    apps["APPDEMO003"]["grok_analysis"] = {
        ...
        "error": False,
        "dimensions": {
            d: agridrone_analysis["dimensions"][d]["grok"]
            for d in agridrone_analysis["dimensions"]
        },
        "overall_recommendation": "APPROVE",
        "recommendation_reasoning": _rec_reasoning("APPROVE", "Grok"),
    ...
    apps["APPDEMO004"] = {
        **cryptosafe,
        ...
        "eligibility_flags": [
            "MALAYSIAN_OWNERSHIP_BELOW_51PCT",
            "LABUAN_IBFC_NOT_SDN_BHD",
            "FOREIGN_MAJORITY_SHAREHOLDER",
        ],
