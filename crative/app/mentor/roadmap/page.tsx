...
import type { RoadmapData, RoadmapNode } from '../../startup/roadmap/page';
...
function loadRoadmap(appId: string): RoadmapData {
    ...
    const defaults: RoadmapData = {
        applicationId: appId, confirmed: false,
        nodes: [
...
function RoadmapContent() {
    ...
    if (!appId) return <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.4)' }}>No application ID provided.</div>;
    if (!roadmap) return <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.4)' }}>Loading…</div>;
    ...
    const removeNode = (id: string) => setDraft((prev) => prev.filter((n) => n.id !== id));
        ...
        <div style={{ maxWidth: '680px', margin: '0 auto', padding: '40px 20px 80px' }}>
            <div style={{ marginBottom: '28px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px', flexWrap: 'wrap' }}>
                    ...
                    {roadmap.confirmed ? (
                        ...
                        <span style={{ padding: '3px 10px', borderRadius: '999px', background: 'rgba(255,165,0,0.1)', color: '#ffa502', border: '1px solid rgba(255,165,0,0.25)', fontSize: '0.75rem', fontWeight: 600 }}>
                            Mentor View — Draft
                        </span>
                    )}
                </div>
                ...
                {!roadmap.confirmed && (
                    <div style={{ marginTop: '8px', padding: '10px 14px', borderRadius: '10px', background: 'rgba(37,217,255,0.05)', border: '1px solid rgba(37,217,255,0.2)', fontSize: '0.82rem', color: 'rgba(200,240,255,0.8)' }}>
                        As a mentor, you can edit and add milestones collaboratively with the startup team. Once both parties agree, confirm the roadmap to lock it.
                    </div>
                )}
            </div>
            ...
            <div>
                {sorted.length === 0 && !editMode && (
                    <div style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.3)' }}>No milestones yet.</div>
                )}
                {sorted.map((node, idx) => (
                    <div key={node.id} style={{ display: 'flex', position: 'relative' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '40px', flexShrink: 0 }}>
                            <div style={{
                                width: '28px', height: '28px', borderRadius: '50%', zIndex: 1, flexShrink: 0,
                            ...
                            {idx < sorted.length - 1 && <div style={{ width: '2px', flex: 1, background: 'rgba(37,217,255,0.2)', minHeight: '40px' }} />}
                        </div>
                        <div style={{ flex: 1, marginLeft: '16px', marginBottom: '24px', background: 'rgba(32,38,56,0.76)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '18px' }}>
                            {editMode && !roadmap.confirmed ? (
                                <>
                                    <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
                                        <input type="date" value={node.date} onChange={(e) => updateNode(node.id, 'date', e.target.value)}
                                        ...
                                        <input type="text" value={node.title} placeholder="Milestone title" onChange={(e) => updateNode(node.id, 'title', e.target.value)}
                                    ...
                                    <textarea value={node.description} placeholder="What needs to be achieved…" onChange={(e) => updateNode(node.id, 'description', e.target.value)} rows={3}
                                ...
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px', flexWrap: 'wrap' }}>
                                        <span style={{ fontWeight: 600, fontSize: '0.95rem' }}>{node.title || <span style={{ color: 'rgba(255,255,255,0.3)' }}>Untitled</span>}</span>
                                        {node.date && (
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '8px' }}>
                    {!editMode ? (
                        ...
                        <>
                            ...
                            <button onClick={() => { setEditMode(false); setDraft(roadmap.nodes); }}
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
                    <div style={{ background: '#0e1220', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', padding: '32px', maxWidth: '420px', width: '100%' }}>
                        ...
                        <p style={{ margin: '0 0 24px', fontSize: '0.88rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.6 }}>
                            Once confirmed, this roadmap is <strong style={{ color: 'white' }}>locked</strong> and cannot be edited. The startup team will follow this plan and upload proof at each milestone. Cradle judges will monitor progress.
                        </p>
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#050508', color: 'white' }}>
            <nav style={{
                ...
                <Link href="/mentor/my-startups" style={{ color: 'white', textDecoration: 'none', fontSize: '0.9rem', fontWeight: 600, letterSpacing: '0.8px', textTransform: 'uppercase' }}>
                    ← Back
                </Link>
                <span style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.45)' }}>Mentor — Roadmap</span>
            </nav>
